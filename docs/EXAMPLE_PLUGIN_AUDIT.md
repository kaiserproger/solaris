...
Date: 2026-07-29; last state check 2026-09-14

This audit evaluates the exact files under `examples/plugins/` against API
`0.6.0`, production adapters, and the shipped wire tests. That directory no
longer exists: the packages live in the independent
`../solaris-default-plugins` checkout (and, for the settlement case, were
replaced by `solaris-settlements` there), so read the rows below as the record
of that audit and not as a description of the current checkout. “Complete” means
...
| `colony-villager-scaffold` | **Removed** (2026-09-12 clean cutover) — superseded by `solaris-settlements` in `../solaris-default-plugins` | Historical: configurable Luau-owned colony metadata and durable role/order intent over an ephemeral villager token | The first extraction is complete: Rust no longer stores colonies or interprets `home`/`hold`; the plugin maps those orders to bounded movement/idle goals and handles typed binding failures. Roles still have no behavior, one player owns at most one durable member record, and general work execution, villager inventory/memory, and durable entity identity remain absent. |
| `geological-mines` | Thin selector, not a complete plugin | Its manifest selects the Rust-owned canonical `realistic_deposits` profile; Luau only prints status | Needs bounded data-driven startup ore descriptors and ownership validation. Per-chunk Luau callbacks are intentionally not proposed because deterministic generation, thread ownership, and throughput remain engine responsibilities. |
| `settlement-prototype` | **Removed** (2026-09-12 clean cutover) — superseded by `solaris-settlements` in `../solaris-default-plugins` | Historical: the manifest chose a Rust-owned prototype and supplied a bounded building/inhabitant/extension plan | Needs a more open startup template-composition and marker schema. Placement, deterministic generation, template decoding, and resident materialization should remain engine primitives; settlement policy and content selection should be plugin data/Luau logic. |
