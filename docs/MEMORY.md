...
- Production worldgen revision 5 consumes explicit `ChunkGeometry` for terrain,
  ores, structures, and biomes. `OverworldRouter` layers continents, uplands,
  land-only ridges and low-relief rivers; each stage has a bounded vertical
  effect. Neighbour-connected tunnel caves stay behind a 32-block surface
  shell. Trees require exact planned support and a stable 5x5 footprint.
