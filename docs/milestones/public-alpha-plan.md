...
Status: **closed for release**. Every automated gate is green. The only unperformed owner-only terrain/playability judgement was explicitly deferred to `v0.0.3-alpha.1` and is not represented as a PASS here; see [`public-alpha3-plan.md`](public-alpha3-plan.md).
...
3. [x] Keep substantial tests beside their focused domain in `*_tests.rs`; stop
   ...
   The three-test door/trapdoor hand-toggle class now lives in focused
   ...
   The following 20-test scheduled-hopper class now lives in focused
   ...
   The following four-test admin-command parsing, permission-aware command
   tree, runtime-control status, and local-development permission class now
   ...
   The following powder-snow equipment/movement-context test now lives in
   ...
   The following bottom-slab collision test now lives in focused
   ...
   The following free-to-solid collision-correction test now lives in focused
   `play/tests/collision_correction_entry.rs`; its unchanged block fixture,
   ...
   The following configured block-loot count test now lives in focused
   `play/tests/block_drop_configured_loot.rs`; its unchanged crop registry,
   carrot item id, dirt loot mapping, fixed count, and resulting runtime stack
   are recorded in
   ...
   The following built-in short-grass drop test now lives in focused
   `play/tests/block_drop_short_grass.rs`; its unchanged air/short-grass
   ...
   The following post-movement pending-teleport confirmation test now lives in
   focused `play/tests/pending_teleport_confirm_behaviour.rs`; its exact move
   ...
   The following unexpected pending-teleport confirmation test now lives in
   focused `play/tests/pending_teleport_unexpected_confirm.rs`; its absent
   ...
   The following outbound write-stall play-loop test now lives in focused
   `play/tests/outbound_write_stall.rs`; its queued animation, stall fixtures,
   exact play-loop arguments and expectations, and timeout counter are
   recorded in
   [`evidence/mc-net-outbound-write-stall-test-extraction.md`](../evidence/mc-net-outbound-write-stall-test-extraction.md).
   The following enchanting owner-commit test now lives in focused
   `play/tests/enchanting_owner_commit.rs`; its pending-before-owner boundary,
   ...
   The following fifteen-bookshelf sharpness test now lives in focused
   `play/tests/enchanting_sharpness.rs`; its exact registries, sharpness clue,
   ...
   The following five-bookshelf fortune test now lives in focused
   ...
   The following enchanting-selection test now lives in focused
   `play/tests/enchanting_selection.rs`; its enchantable pickaxe, lapis and XP
...
1. [x] Complete the clock, worldgen, natural-spawn, item transaction, restart, and
   ...
   and is carried by [`public-alpha3-plan.md`](public-alpha3-plan.md).
...
- [x] Replace the `mc-script` targeted-event backpressure test's scheduler
  yield with an exact poll of the bounded send future. The test now proves
