...
- Online-mode/session auth, duplicate-name handling, permissions, chat policy,
  ...
  start without persistent chunk streaming/storage, plus sidecar `version.json`
  metadata warnings for stale or malformed `data.vanilla_data_dir` extracts.
  That is check-output evidence only; it is not online-mode auth, broad
  filesystem/permissions validation, a public deployment gate,
