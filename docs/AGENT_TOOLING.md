...
| `standard-pack` | The core harness dispatches to `../solaris-default-plugins/tools/run_integration.py`; its sibling-owned tests (`sources/integration-tests/tests/plugin_standard_pack.rs`, `wasm_economy_payment.rs`, and `plugin_live_workload.rs`) verify the first-party components, including a real TCP/Wasm payment across token eviction, reload, and world restart. The sibling runner then stages the strict five-package deployment and checks real `mc-server --check` startup. Source bytes are verified against the sibling build; local vanilla sidecars and the guest build target are required. `SOLARIS_DEFAULT_PLUGINS_ROOT` selects the sibling checkout. `--real-client` additionally runs the sibling-owned graphical `/money` scenario under Xvfb. `python3 -m tools.harness run standard-pack` |
| `settlement-cargo` | The core harness dispatches to the same sibling runner, which checks the shipped `solaris-settlements` component, runs the sibling-owned `settlement_component_haul` and `settlement_wasm_readiness` host scenarios, then runs the owning native `mc-net` API suites for resident orders, warehouses, construction, and persisted deposits. Requires `../solaris-default-plugins` and local vanilla sidecars. `python3 -m tools.harness run settlement-cargo` |
