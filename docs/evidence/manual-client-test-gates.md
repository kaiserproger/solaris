...
| Seed `712816` fresh-world visual/playable and restart gate | Partly automatable; terrain feel remains owner-observed | `mc-worldgen` plus the playable route. The deterministic [`2048x2048`-block height, biome, and vegetation mosaics](worldgen-mosaics.md) are checked in, and the [agent-run graphical preflight](worldgen-seed-712816-preflight.md) passed on a fresh exact-seed world without operators or debug commands. Close the remaining gate with ordinary owner traversal, an owner-recorded disposition of visual/playability findings, and the separate restart/rejoin checkpoint. |
...
| Server-only and client-required plugin matrix | Automated real-client gate passed 2026-08-27 | `mc-script`, server adapters, and the client Loader. Closed on the current tree: the no-Loader real-client profile entered Play with shipped server-only `basic-economy` and opened its `/economy` menu; the same profile was rejected before Play by `examples/loader-live-gate` with an explicit Solaris Loader requirement; fresh isolated Fabric, NeoForge, and Forge Loader clients each accepted the exact permission prompt, cached both exact owner bundle identities, exercised both owner screens/buttons/grants, and remained in Play. Evidence is in [`phase5-plugin-fixtures-compatibility-2026-08-27.md`](phase5-plugin-fixtures-compatibility-2026-08-27.md). |
