...
Next: close the common vanilla mob and villager/village loop on the unified
`main` tree. The implementation must keep behavior data-driven and owner-authoritative
so plugins can replace goals, schedules, professions, trades, memories, sensors,
reputation and village policies without forking entity storage or wire code. Prove
natural spawn, navigation, work/rest/social/combat transitions, persistence and
client-visible state before beginning the large Solaris-versus-vanilla benchmark matrix.
