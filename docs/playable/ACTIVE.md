...
Next: prove partial pickup and overflow conservation. A full or near-full
inventory must accept only the admissible item count; the remainder must retain
its exact entity identity, item identity and count, while stale owner state or
disconnect cannot partially debit either side. This checkpoint has unit and wire
legs and no client-rig dependency.
