...
Next: prove chest max-stack metadata and malformed edge conservation. Pickup,
quick-move and drag must obey item-specific stack limits, while impossible client
prediction or stale state must fail closed with authoritative chest storage and
cursor unchanged. This checkpoint has unit, wire and replay/negative legs and no
client-rig dependency.
