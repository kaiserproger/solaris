...
assignment, player-specific `TRADING`, and direct `VILLAGER_HURT` gossip are closed.
Gossip now belongs to every villager rather than merchant state: accepted player melee
adds exact `MINOR_NEGATIVE +25`, while trade remains `+2`; both decay with their own
26.1.2 rates and feed the same UUID-specific price/debit path. Continue with gossip
transfer, `VILLAGER_KILLED`/major-negative propagation, remaining event types, and
