...
Next: continue with village defence and the remaining species-specific mob loop on the
unified `main` tree. The first bounded population slice is closed: generated settlements
publish one persisted vacant HOME, two adult villagers can consume exactly six dropped
bread through one atomic courtship commit, and one delayed baby survives restart without
duplication. This is not full villager breeding: carrots, potatoes, beetroot, farming,
food sharing, arbitrary POI discovery/path reachability, broader population balancing,
and additional births remain deferred. Merchant trading, bounded restock, fail-closed
job-site profession assignment, all five gossip types, and production zombie-villager
curing are closed. The curing slice intentionally omits vanilla bed/iron-bar acceleration,
so conversion advances one tick per lifecycle tick. Pillager crossbow combat is now a real
species-specific path: the canonical crossbow is client-visible, aim/charge/charged state is
retained by the entity owner, charging metadata is published, and the delayed shot reuses the
authoritative owner-attributed arrow pipeline. A bounded iron-golem panic-defence slice is
also closed: recently rested adult villagers persist the 26.1.2 sleep/golem memories, three
eligible villagers can create one collision-validated persisted golem, and the golem pursues
nearby hostiles except creepers through the normal goal, damage, death, loot, velocity and
tracking-event paths. This is not complete village defence: actual villager bed pose,
five-villager gossip spawning, exact Java spawn/attack RNG, player-created anger and
reputation, flowers, repair/crackiness, raids and Hero of the Village pricing remain open.
Replace each remaining explicit `UnsupportedSpecial` profile with its real attack path or
keep it visibly blocked; do not silently route those mobs through generic melee. The existing
performance and soak matrix may run in parallel, but does not convert focused playable
evidence into replacement-readiness.

## Recent Checkpoints — through 2026-07-29
