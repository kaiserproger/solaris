...
Next: prove the dropped-item despawn deadline across restart. Persisted
`spawn_tick` must reconstruct the exact remaining lifetime; the item must neither
despawn early nor live forever before or after restart, and stale deadline-index
IDs must remain bounded. This checkpoint has unit, wire and persistence legs and
no client-rig dependency.
