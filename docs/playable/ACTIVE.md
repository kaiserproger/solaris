...
Next: prove crafting-table max-craft and cursor conservation. Normal take,
shift-click/max craft, full inventory and stale state must conserve every input,
output and cursor stack exactly, with no partial craft or duplicated remainder.
This checkpoint has unit and wire legs and no client-rig dependency.
