...
assignment, player-specific trade/hurt/kill gossip, and bounded transfer of all five
vanilla gossip types are closed. Transfer runs only for `Idle`/`Meet` initiators within
`distance² <= 5`, applies the shared 1,200-tick cooldown and commits both villagers
atomically. The exact `ZOMBIE_VILLAGER_CURED` state event now adds `MINOR_POSITIVE +25`
and `MAJOR_POSITIVE +20`, but Solaris still lacks the production zombie-villager curing
and conversion path that would emit it. Implement that runtime path before claiming the
positive event complete, then continue with village population/defence policy; Hero of
the Village pricing is also still open. Replace each explicit `UnsupportedSpecial`
profile with its real attack path or keep it visibly blocked; do not silently route those
mobs through generic melee. The existing performance and soak matrix may run in
parallel, but does not convert focused playable evidence into replacement-readiness.
