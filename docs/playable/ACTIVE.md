...
Next: prove shield angle/timing and axe-disable behavior. A timely front-angle
block must prevent damage and consume exact durability; back-angle or late use
must not block, while an axe hit must disable the shield for the authoritative
duration and publish the resulting state. This checkpoint has unit and wire legs
and no client-rig dependency.
