...
Next: complete the villager gameplay and remaining species-specific mob loop on the
unified `main` tree. Add the 26.1.2 merchant menu/offers/select-trade wire contract,
authoritative trade inventory/XP/restock, job-site profession reassignment,
gossip/reputation and village population/defence policy. Then replace each explicit
`UnsupportedSpecial` profile with its real attack path or keep it visibly blocked;
do not silently route those mobs through generic melee. Only after these gates pass
begin the large Solaris-versus-vanilla benchmark matrix.
