...
assignment, player-specific trade/hurt gossip, and bounded transfer of the currently
implemented `TRADING`/`MINOR_NEGATIVE` entries are closed. Transfer runs only for
`Idle`/`Meet` initiators within `distance² <= 5`, applies the shared 1,200-tick cooldown
and commits both villagers atomically. Continue with `VILLAGER_KILLED` / major-negative
propagation, positive gossip types and remaining events, then village population/defence
policy; Hero of the Village pricing is also still open. Replace each explicit
`UnsupportedSpecial` profile with its real attack path or keep it visibly blocked; do not
silently route those mobs through generic melee. The existing performance and soak
matrix may run in parallel, but does not convert focused playable evidence into
