...
unified `main` tree. Merchant trading, bounded restock and the first fail-closed job-site
profession assignment are closed: an unemployed adult claims Toolsmith only from a
loaded `minecraft:smithing_table`, receives the canonical merchant catalog and publishes
one metadata transition. Add gossip/reputation and village population/defence policy.
Then replace each explicit `UnsupportedSpecial` profile with its real attack path or keep
it visibly blocked; do not silently route those mobs through generic melee. The existing
