...
so conversion advances one tick per lifecycle tick. Hero of the Village pricing,
iron-golem spawning/combat, raids, and broader village defence remain open. Replace each
explicit `UnsupportedSpecial` profile with its real attack path or keep it visibly
blocked; do not silently route those mobs through generic melee. The existing
