...
Next: integrate every outstanding worktree branch newer than `main`, run the
combined workspace validation, and establish tag-driven release automation. A tag
must run the full CI gate, publish the server binary as a release asset, and expose
a fail-closed installer script for supported Linux targets.
