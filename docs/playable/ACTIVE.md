...
registries, pickaxe/table fixtures, XP seed, and exact ten-property payload are
recorded in
...
unchanged pending id, mismatched confirmation and retained state, matching
confirmation, and cleared state are recorded in
...
The complete unexpected pending-teleport confirmation test has moved to
`crates/mc-net/src/play/tests/pending_teleport_unexpected_confirm.rs`. Its
unchanged absent pending state, confirmation id, `Unexpected` result, and
still-absent state are recorded in
...
synthetic-solid fixture, covered numeric state id, mismatched identity,
full-cube collision, and published collision-context coverage are recorded in
[`../evidence/mc-net-unrelated-state-collision-test-extraction.md`](../evidence/mc-net-unrelated-state-collision-test-extraction.md).
...
north-facing bottom-stair state, occupied north upper step, empty south half
above the lower step, exact poses, and published collision-context coverage
are recorded in
...
state identity, published block, center pose, full-cube collision, and
...
exact `0.9F` boundary, outside/inside poses, and published collision-context
...
- The Loader client gate now has an isolated MCP launcher for Fabric, NeoForge,
  ...
  On 2026-08-27 the gate was made reproducible as `tools/run-loader-live-gate.py`
  and rerun on fresh isolated Fabric, NeoForge, and Forge profiles. All three
  entered Play after the exact permission prompt, cached the same Ruby/Sapphire
  bundle hashes, opened both owner screens, confirmed both owner-scoped buttons,
  granted one then two exact carrier items, and remained in Play. The fresh Forge
  run is `.analysis/loader-live-gate/runs/20260827T092505-forge`; Fabric is
  `20260827T092358-fabric`; NeoForge is `20260827T092432-neoforge`.
...
- Solaris Loader now has one protocol-1 manifest/ack contract across Fabric,
  ...
  client-verified. NeoForge has the same client-verified Configuration, grant,
  placement, distinct projection, visual, survival-drop, and exact-pickup
  evidence. The 2026-08-27 reproducible compatibility gate additionally passes
  fresh isolated Fabric, NeoForge, and Forge processes on the exact two-owner
  permission/bundle/screen/button/grant/Play path. Forge therefore no longer
  remains an unrun platform; manual PrismLauncher ownership inspection is not a
  separate protocol acceptance requirement for the public-alpha Loader matrix.
