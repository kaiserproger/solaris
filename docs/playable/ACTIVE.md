...
Next: continue the villager gameplay and remaining species-specific mob loop on the
unified `main` tree. Merchant trading, bounded restock, fail-closed job-site profession
assignment, and the first player-specific `TRADING` gossip slice are closed. Successful
trades add exact 26.1.2 reputation `+2` up to `25`; it decays by `2` after 24,000 ticks,
values below `2` disappear, and only the matching player UUID receives the projected
price and discounted debit. Continue with gossip transfer/non-trading event types and
village population/defence policy; Hero of the Village pricing is also still open. Then
replace each explicit `UnsupportedSpecial` profile with its real attack path or keep it
visibly blocked; do not silently route those mobs through generic melee. The existing
performance and soak matrix may run in parallel, but does not convert focused playable
evidence into replacement-readiness.

## Recent Checkpoints — through 2026-07-27
...
| Toolsmith player trading gossip | A successful authoritative trade now records the exact 26.1.2 `TRADING` gossip value `+2` for the actor UUID, capped at `25`. Merchant offer packets and payment selection derive only that customer's special price; the owner commit recomputes the live session UUID, so another player neither receives nor can spend the discount. Trading gossip decays by `2` once at least 24,000 ticks have elapsed, drops entries below `2`, and persists with its last-decay timestamp through entity checkpoints. The ledger is deliberately bounded to 64 player UUIDs; a full ledger never rejects or duplicates the underlying trade. | local decompiled 26.1.2 gossip/price oracle inspected; entity reputation/price/decay matrix `3/3`; merchant authority stale-replay slice `3/3`; villager brain cadence/restock/decay suite `7/7`; entity checkpoint restart `1/1`; full `mc-entity`, full `mc-net`, and full worldgen harness PASS; workspace Clippy `-D warnings`, formatter, diff-check and code-health `0 fail / KEEP` PASS. This is focused container/owner/persistence evidence, not a dedicated raw-TCP personal-price or manual-client gate. Gossip transfer, non-trading gossip types and Hero of the Village remain open. |
