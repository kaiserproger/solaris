...
unified `main` tree. The 26.1.2 merchant menu/offers/select-trade wire contract and
authoritative trade inventory, XP, use-count, input persistence and bounded job-site
restock are closed. Add job-site profession reassignment, gossip/reputation and village
population/defence policy. Then replace each explicit `UnsupportedSpecial` profile with
its real attack path or keep it visibly blocked; do not silently route those mobs through
generic melee. Only after these gates pass begin the large Solaris-versus-vanilla
benchmark matrix.
