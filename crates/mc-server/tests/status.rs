...
#[tokio::test]
async fn login_intent_does_not_crash_server() {
    // M1.d will turn this into a real login flow. For M1.c we just
    // verify the listener stays up after a login-intent handshake.
    let addr = start_server("M1.c").await;
    let mut stream = TcpStream::connect(addr).await.unwrap();
    write_frame(
        &mut stream,
        &Handshake {
    ...
    .await;
    // Server should accept the handshake and then drop the connection
    // because login is not yet implemented. The read will return 0.
    let mut scratch = [0u8; 1];
    let n = tokio::time::timeout(std::time::Duration::from_secs(2), stream.read(&mut scratch))
        .await
        .expect("server did not close on its own within 2s")
        .unwrap_or(0);
    assert_eq!(n, 0);

    // A second, independent client should still be able to ping.
    ...
    .await;
    write_frame(&mut other, &StatusRequest).await;
    let mut frame = read_one_frame(&mut other).await;
    assert_eq!(frame.id, StatusResponse::ID);
