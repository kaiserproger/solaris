...
use mc_protocol::packets::play::{
    ...
    ClientboundSetHealth, ConfirmTeleportation, EntityAnimation, EntityEvent, GameEvent,
    InteractionHand, LevelChunkWithLight, MoveEntityPosRot, MovePlayerFlags, RemoveEntities,
    ServerboundAttack, ServerboundChatCommand, ServerboundKeepAlive, ServerboundMovePlayerPos,
    ServerboundMovePlayerPosRot, ServerboundUseItem, SetCenterChunk, SetEntityMotion,
...
async fn survival_shield_blocks_frontal_zombie_damage() {
    ...
    let zombie = wait_for_zombie_spawn(&mut client, zombie_type_id).await;
    wait_for_blocked_zombie_damage(&mut client, zombie.entity_id).await;
}
...
async fn wait_for_blocked_zombie_damage(client: &mut Client, entity_id: i32) {
    ...
    loop {
        ...
        if frame.id == EntityAnimation::ID {
            ...
            assert!(
                ...
                "frontal shield use should keep health unchanged"
            );
            if saw_swing {
                return;
            }
