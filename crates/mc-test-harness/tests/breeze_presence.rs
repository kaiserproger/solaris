...
#[tokio::test]
async fn embedded_breeze_wind_charge_damages_triggers_knockback_and_discards_over_tcp() {
    ...
    let breeze_type_id = entity_type_id(&entity_types, "minecraft:breeze");
    ...
    let bound = mc_net::bind(cfg).await.expect("bind breeze server");
    ...
    client
        .write_packet(&ServerboundChatCommand {
            command: format!(
                ...
                breeze_position.0, breeze_position.1, breeze_position.2
            ),
        })
        .await
        .expect("summon breeze");
    let breeze = wait_for_entity_type_spawn_at(&mut client, breeze_type_id, breeze_position).await;

    let deadline = tokio::time::Instant::now() + Duration::from_secs(12);
    let mut wind_charge_id = None;
    let mut saw_direct_damage = false;
    let mut saw_trigger_explosion = false;
    let mut saw_remove = false;
    while wind_charge_id.is_none() || !saw_direct_damage || !saw_trigger_explosion || !saw_remove {
        let remaining = deadline.saturating_duration_since(tokio::time::Instant::now());
        if remaining.is_zero() {
            panic!(
                "breeze wind-charge lifecycle did not converge: breeze={} wind_charge={wind_charge_id:?} damage={saw_direct_damage} explosion={saw_trigger_explosion} removed={saw_remove}",
                breeze.entity_id,
            );
        }
        let frame = client.read_frame_with_timeout(remaining).await.unwrap_or_else(|error| {
            panic!(
                "breeze wind-charge TCP lifecycle failed: {error}; breeze={} wind_charge={wind_charge_id:?} damage={saw_direct_damage} explosion={saw_trigger_explosion} removed={saw_remove}",
                breeze.entity_id,
            )
        });
        ...
        if frame.id == AddEntity::ID {
            let packet =
                AddEntity::decode(&mut frame.body.clone()).expect("decode breeze wind charge");
            if packet.entity_type_id == wind_charge_type_id {
                wind_charge_id.get_or_insert(packet.entity_id);
            }
        } else if frame.id == ClientboundSetHealth::ID {
            let health = ClientboundSetHealth::decode(&mut frame.body.clone())
                .expect("decode breeze wind-charge player damage");
            saw_direct_damage |= (health.health - 19.0).abs() < f32::EPSILON;
        } else if frame.id == ClientboundExplode::ID {
            let explosion = ClientboundExplode::decode(&mut frame.body.clone())
                .expect("decode breeze wind-charge explosion");
            saw_trigger_explosion |=
                (explosion.radius - 3.0).abs() < f32::EPSILON && explosion.block_count == 0;
        } else if frame.id == RemoveEntities::ID {
            let removed = RemoveEntities::decode(&mut frame.body.clone())
                .expect("decode breeze wind-charge removal");
            if let Some(wind_charge_id) = wind_charge_id {
                saw_remove |= removed.entity_ids.contains(&wind_charge_id);
            }
        }
    }
    ...
    tokio::time::timeout(Duration::from_secs(5), server)
        .await
        .expect("breeze server shutdown timeout")
        .expect("breeze server task")
        .expect("breeze server result");
}
...
async fn drain_until_chunk(client: &mut Client, target: (i32, i32)) {
    ...
    loop {
        let frame = client
            ...
            .await
            .expect("drain breeze chunks");
        if handle_keepalive(client, frame.id, &frame.body).await {
            continue;
        }
        if frame.id == LevelChunkWithLight::ID {
            let packet =
                LevelChunkWithLight::decode(&mut frame.body.clone()).expect("decode breeze chunk");
...
async fn wait_for_entity_type_spawn_at(
    ...
    loop {
        let frame = client
            ...
            .await
            .expect("wait for breeze spawn");
        if handle_keepalive(client, frame.id, &frame.body).await {
            continue;
        }
        if frame.id == AddEntity::ID {
            let packet = AddEntity::decode(&mut frame.body.clone()).expect("decode breeze spawn");
...
async fn handle_keepalive(client: &mut Client, id: i32, body: &bytes::Bytes) -> bool {
    ...
    let keepalive =
        ClientboundKeepAlive::decode(&mut body.clone()).expect("decode breeze keepalive");
    client
        .write_packet(&ServerboundKeepAlive { id: keepalive.id })
        .await
        .expect("answer breeze keepalive");
    true
}
