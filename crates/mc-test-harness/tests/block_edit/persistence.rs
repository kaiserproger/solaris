...
async fn embedded_save_restart_rejoin_preserves_inventory_and_edited_block() {
    ...
    assert_eq!(
        landed, crafting_table_state,
        "edited block should survive restart"
    );
...
async fn embedded_non_op_shutdown_restart_preserves_survival_edit_and_inventory() {
    ...
    let stone_state = embedded_block_state(&data, "minecraft:stone");
    let jungle_log_id = embedded_item_id(&data, "minecraft:jungle_log");
    let world_dir = tempfile::tempdir().expect("temp world");
    ...
    for x in -1..=1 {
        world
            ...
            .expect("seed stable log resource")
    ...
    let hand_log_ticks = vanilla_stop_destroy_ticks(2.0, 1.0, true);
    ...
    client
        ...
        .await
        .expect("place survival-mined log");
    wait_for_block_update(
        &mut client,
        (2, surface_y + 1, 0),
        jungle_log_state.0 as i32,
    )
    .await;
    wait_for_slot_stack(&mut client, jungle_log_id, 1).await;
    ...
    assert_eq!(
        persisted_block, jungle_log_state,
    ...
    let restored = wait_for_inventory_content(&mut client, |pkt| {
        pkt.container_id == 0
            && pkt.items.get(36).is_some_and(|stack| {
                stack.item_id == jungle_log_id && stack.count == 1 && stack.damage.is_none()
            })
    })
    .await;
    assert_eq!(restored.items[36].item_id, jungle_log_id);
