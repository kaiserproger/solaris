...
use mc_protocol::packets::play::{
    ...
    ClientboundSetHealth, ClientboundSetTime, ClientboundUpdateEntityEffect, ConfirmTeleportation,
...
const LEVITATION_EFFECT_ID_26_1_2: u32 = 24;

#[tokio::test]
async fn embedded_shulker_bullet_homes_damages_levitates_and_discards_over_tcp() {
    ...
    let shulker_type_id = entity_type_id(&entity_types, "minecraft:shulker");
    let bullet_type_id = entity_type_id(&entity_types, "minecraft:shulker_bullet");
    let shutdown = mc_net::ShutdownHandle::default();
    let cfg = mc_net::ServerConfig {
        bind_address: "127.0.0.1:0".parse().unwrap(),
        motd: "Phase 4 shulker TCP".into(),
        max_players: 2,
    ...
    let bound = mc_net::bind(cfg).await.expect("bind shulker server");
    let addr = bound.local_addr().expect("shulker server address");
    let server = tokio::spawn(async move { bound.serve().await });

    let (mut client, spawn) = connect_to_play(addr, "ShulkerTcp").await;
    drain_until_chunk(&mut client, (0, 0)).await;
    let shulker_position = (spawn.x, spawn.y, spawn.z + 6.0);
    client
        .write_packet(&ServerboundChatCommand {
            command: format!(
                "summon minecraft:shulker {} {} {}",
                shulker_position.0, shulker_position.1, shulker_position.2
            ),
        })
        .await
        .expect("summon shulker");
    let shulker =
        wait_for_entity_type_spawn_at(&mut client, shulker_type_id, shulker_position).await;

    let deadline = tokio::time::Instant::now() + Duration::from_secs(12);
    let mut bullet_id = None;
    let mut saw_damage = false;
    let mut saw_levitation = false;
    let mut saw_bullet_remove = false;
    while bullet_id.is_none() || !saw_damage || !saw_levitation || !saw_bullet_remove {
        let remaining = deadline.saturating_duration_since(tokio::time::Instant::now());
        if remaining.is_zero() {
            panic!(
                "shulker bullet lifecycle did not converge: shulker={} bullet={bullet_id:?} damage={saw_damage} levitation={saw_levitation} removed={saw_bullet_remove}",
                shulker.entity_id,
            );
        }
        let frame = client.read_frame_with_timeout(remaining).await.unwrap_or_else(|error| {
            panic!(
                "shulker bullet TCP lifecycle failed: {error}; shulker={} bullet={bullet_id:?} damage={saw_damage} levitation={saw_levitation} removed={saw_bullet_remove}",
                shulker.entity_id,
            )
        });
        ...
        if frame.id == AddEntity::ID {
            let packet = AddEntity::decode(&mut frame.body.clone()).expect("decode shulker bullet");
            if packet.entity_type_id == bullet_type_id {
                bullet_id.get_or_insert(packet.entity_id);
            }
        } else if frame.id == ClientboundSetHealth::ID {
            let health = ClientboundSetHealth::decode(&mut frame.body.clone())
                .expect("decode shulker bullet player damage");
            saw_damage |= health.health <= 16.0;
        } else if frame.id == ClientboundUpdateEntityEffect::ID {
            let effect = ClientboundUpdateEntityEffect::decode(&mut frame.body.clone())
                .expect("decode shulker levitation");
            saw_levitation |= effect.effect_id.raw() == LEVITATION_EFFECT_ID_26_1_2
                && effect.amplifier == 0
                && effect.duration_ticks == 200;
        } else if frame.id == RemoveEntities::ID {
            let removed = RemoveEntities::decode(&mut frame.body.clone())
                .expect("decode shulker bullet removal");
            if let Some(bullet_id) = bullet_id {
                saw_bullet_remove |= removed.entity_ids.contains(&bullet_id);
            }
        }
    }
    ...
    tokio::time::timeout(Duration::from_secs(5), server)
        .await
        .expect("shulker server shutdown timeout")
        .expect("shulker server task")
        .expect("shulker server result");
}
...
async fn drain_until_chunk(client: &mut Client, target: (i32, i32)) {
    ...
    loop {
        let frame = client
            ...
            .await
            .expect("drain shulker chunks");
        if handle_keepalive(client, frame.id, &frame.body).await {
            continue;
        }
        if frame.id == LevelChunkWithLight::ID {
            let packet =
                LevelChunkWithLight::decode(&mut frame.body.clone()).expect("decode shulker chunk");
...
async fn wait_for_entity_type_spawn_at(
    ...
    loop {
        let frame = client
            ...
            .await
            .expect("wait for shulker spawn");
        if handle_keepalive(client, frame.id, &frame.body).await {
            continue;
        }
        if frame.id == AddEntity::ID {
            let packet = AddEntity::decode(&mut frame.body.clone()).expect("decode shulker spawn");
...
async fn handle_keepalive(client: &mut Client, id: i32, body: &bytes::Bytes) -> bool {
    ...
    let keepalive =
        ClientboundKeepAlive::decode(&mut body.clone()).expect("decode shulker keepalive");
    client
        .write_packet(&ServerboundKeepAlive { id: keepalive.id })
        .await
        .expect("answer shulker keepalive");
    true
}
