...
fn injured_animal_runs_along_knockback_then_returns_to_walking_speed() {
    // 26.1.2 PanicGoal multipliers: cow 2.0, sheep/pig 1.25, chicken 1.4.
    for (type_name, expected_panic_speed) in [
        ("minecraft:cow", 4.0),
        ("minecraft:sheep", 2.5),
        ("minecraft:pig", 2.5),
        ("minecraft:chicken", 2.8),
    ] {
        let mut store = EntityStore::new();
        let mut animal = SpawnEntity::new(11, type_name, Vec3::new(0.5, 64.0, 0.5));
        ...
        assert!(fleeing.position.x > 0.5, "{type_name}");
        assert!(
            (fleeing.velocity.x - expected_panic_speed).abs() < 1.0e-6,
            "{type_name}"
        );
