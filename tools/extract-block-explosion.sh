#!/usr/bin/env bash
# Extract per-state explosion resistance from the bundled vanilla server.
# Output is local sidecar data, not source.

set -euo pipefail

REPO_ROOT="$(cd "$(dirname "$0")/.." && pwd)"
BUNDLE_JAR="${1:-$REPO_ROOT/.analysis/server.jar}"
OUT_JSON="$REPO_ROOT/data/vanilla/reports/block_explosion.json"
...
INNER_JAR="$(find "$TMP/bundle/META-INF/versions" -name 'server-*.jar' -print -quit)"
if [[ -z "$INNER_JAR" ]]; then
...
echo "[2/4] Compiling ExplosionExtractor..."
mkdir -p "$TMP/classes"
"$JAVAC" -d "$TMP/classes" -cp "$CP" \
  "$REPO_ROOT/tools/extract-block-explosion/ExplosionExtractor.java"

echo "[3/4] Running extractor..."
mkdir -p "$(dirname "$OUT_JSON")"
"$JAVA" -cp "$TMP/classes:$CP" ExplosionExtractor "$OUT_JSON"
