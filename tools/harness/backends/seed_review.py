...
from tools.harness import runtime as helpers
from tools.harness.mcp import McpClient
from tools.harness.runtime import REPO_ROOT
SEED = 712816
...
def run(timeout_seconds: float) -> dict[str, Any]:
    ...
    evidence_root = os.environ.get("SOLARIS_VALIDATION_RUN_DIR")
    run_dir = (
        Path(evidence_root) / "seed-review"
        if evidence_root
        else REPO_ROOT / ".analysis" / "seed-owner-review" / run_id
    )
    ...
    try:
        ...
        helpers.wait_port(
            server_port, min(timeout_seconds, 90.0), server, log_path=run_dir / "server.log"
        )
        ...
        client_process, client_log = helpers.start_client(
            game_dir=game_dir,
            log_path=run_dir / "client.log",
            token=token,
            mcp_port=mcp_port,
            username="SeedOwnerReview",
            display=display,
        )
