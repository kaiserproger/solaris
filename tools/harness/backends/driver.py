...
SETTLEMENT_CHAIN_NAME = "regsville"
SETTLEMENT_CHAIN_BLUEPRINT = "solaris:house_small"
SETTLEMENT_CHAIN_GIVE_CAP = 255
SETTLEMENT_CHAIN_TELEPORT_Y = 250
# The authored `structures/house_small.toml` blueprint resource totals; `/give`
# is the operator setup route and caps one call at 255.
SETTLEMENT_CHAIN_MATERIALS = (
    ("minecraft:oak_planks", 314),
...
# The committed house sits at the player anchor; the scan half-extents cover the
# 11x7x9 blueprint in either anchor convention, split into two boxes because one
# scan is capped at 4096 cells.
SETTLEMENT_CHAIN_SCAN_HALF_X = 11
SETTLEMENT_CHAIN_SCAN_HALF_Z = 9
SETTLEMENT_SITE_CHAT_LINE = re.compile(
    r"(site_[a-z0-9_]+) ([a-z_]+) origin (-?\d+),(-?\d+),(-?\d+) "
    r"size \d+,\d+,\d+ buildings=\d+"
)
SETTLEMENT_PROJECT_CHAT_LINE = re.compile(
    r"([a-z0-9_]+) projected \("
    + re.escape(SETTLEMENT_CHAIN_BLUEPRINT)
    + r", (\d+) stages\)\. Fund it: /settlement fund "
    + SETTLEMENT_CHAIN_NAME
    + r" ([a-z0-9_]+)"
)
SETTLEMENT_COMMIT_CHAT_LINE = re.compile(
    r"([a-z0-9_]+) committed \("
    + re.escape(SETTLEMENT_CHAIN_BLUEPRINT)
    + r"\) at revision (\d+)\."
)
SETTLEMENT_INFO_CHAT_LINE = re.compile(
    re.escape(SETTLEMENT_CHAIN_NAME)
    + r" \| \S+ \S+ \S+ tier=\S+ pop=(\d+) houses=(\d+) jobs=(\d+) "
    + r"food=(\d+) money=(\d+) specs=\S+"
)
...
def core_replay_provenance(run_dir: Path) -> dict[str, Any]:
    ...
    properties_path = Path(
        os.environ.get("SOLARIS_LOADER_ROOT", repo_root().parent / "solaris-loader")
    ).expanduser().resolve() / "gradle.properties"
    client_version = ""
...
def run_bridge_scenario(
    ...
    if scenario_id == "m94-colony-villager-live-qa":
        return run_colony_villager_live_qa_scenario(
            client,
            run_dir,
            scenario_id,
            server_addr,
    ...
    if scenario_id == SETTLEMENT_CHAIN_SCENARIO:
        return run_settlement_chain_scenario(
            client,
            run_dir,
            scenario_id,
            server_addr,
...
def run_colony_villager_live_qa_scenario(
    client: AgentClient,
    run_dir: Path,
    scenario_id: str,
    server_addr: str,
    ...
    def recent_chat(observation: dict[str, Any]) -> list[str]:
        value = observation.get("recent_chat")
    ...
    def observe_client() -> dict[str, Any]:
        observation = call_and_record(
            client,
            transcript,
            "observe",
            {},
        ...
        if not observation.get("in_play"):
            raise RuntimeError("client left Play state during colony QA")
        return observation

    def wait_for_exact_chat(
        expected: list[str],
        baseline_counts: dict[str, int],
    ) -> dict[str, Any]:
        deadline = time.monotonic() + timeout_seconds
        while True:
            observation = observe_client()
            lines = recent_chat(observation)
            if all(lines.count(message) > baseline_counts[message] for message in expected):
                return observation
            remaining = deadline - time.monotonic()
            if remaining <= 0.0:
                missing = [
                    ...
                    if lines.count(message) <= baseline_counts[message]
                ]
                raise TimeoutError(f"exact colony chat did not arrive: {missing}")
            ...
            event_timeout = min(remaining, 10.0)
            call_and_record(
                client,
                transcript,
                "wait_state_change",
    ...
    def issue_command(command: str, expected: list[str]) -> dict[str, Any]:
        before = observe_client()
        before_lines = recent_chat(before)
        baseline_counts = {message: before_lines.count(message) for message in expected}
        exact_commands.append("/" + command)
        call_and_record(
            client,
            transcript,
            "send_chat",
            {"message": command, "command": True},
            timeout_seconds,
        )
        call_and_record(
            client,
        ...
        after = wait_for_exact_chat(expected, baseline_counts)
        command_observations.append(
            {
                "command": "/" + command,
                "expected_chat": expected,
                "observed_chat": [
                    line for line in recent_chat(after) if line in expected
                ],
            }
        )
        return after

    def visible_villagers(payload: dict[str, Any]) -> list[dict[str, Any]]:
        entities = payload.get("entities")
        if not isinstance(entities, list):
            raise RuntimeError("list_entities did not return entities")
        return [
            entity
            for entity in entities
            if isinstance(entity, dict)
            and entity.get("entity_type") == "minecraft:villager"
        ]

    call_and_record(client, transcript, "ping", {}, timeout_seconds)
    play_state = wait_for_existing_or_explicit_connection(
        client,
        transcript,
        server_addr,
        timeout_seconds,
    ...
    if not is_in_play(play_state):
        raise RuntimeError("client did not reach Play state for colony QA")
    initial_observation = wait_for_exact_chat(
        ["Starter Colony plugin ready."],
        {"Starter Colony plugin ready.": 0},
    )
    call_and_record(
        client,
        transcript,
    ...
    summon_message = "Summoned minecraft:villager"
    issue_command("summon villager", [summon_message])
    issue_command("summon villager", [summon_message])
    summoned_entities = call_and_record(
        client,
        transcript,
        "list_entities",
        {"radius": 24.0, "limit": 64},
        timeout_seconds,
    )
    summoned_villagers = visible_villagers(summoned_entities)
    if len(summoned_villagers) < 2:
        raise RuntimeError(
            f"two summoned villagers were not client-visible: count={len(summoned_villagers)}"
        )

    durable_message = "Villager recruitment recorded durably by the Luau plugin."
    issue_command("colony recruit worker", [durable_message])
    issue_command("colony recruit guard", [durable_message])
    member_one_initial = (
        "Starter Colony member 1: status=active, role=worker, "
        "order=home, generation=2."
    )
    member_two_initial = (
        "Starter Colony member 2: status=active, role=guard, "
        "order=follow, generation=2."
    )
    issue_command("colony status", [member_one_initial, member_two_initial])

    invalid_action = "Colony command rejected: invalid bounded action."
    issue_command("colony role guard 1", ["Applied Luau order follow."])
    issue_command("colony order home 1", ["Applied Luau order home."])
    requested_command_contract_passed = True

    issue_command("colony role farmer 1", ["Applied Luau order home."])
    issue_command("colony order hold 1", ["Applied Luau order hold."])
    issue_command("colony dismiss 2", ["Member 2 dismissed: slot released."])

    member_one_held = (
        "Starter Colony member 1: status=active, role=farmer, "
        "order=hold, generation=6."
    )
    before_post_dismiss = observe_client()
    member_two_count_before = recent_chat(before_post_dismiss).count(member_two_initial)
    post_dismiss = issue_command("colony status", [member_one_held])
    member_two_count_after = recent_chat(post_dismiss).count(member_two_initial)
    if member_two_count_after != member_two_count_before:
        raise RuntimeError("dismissed member 2 was still emitted by colony status")

    issue_command("colony recruit guard", [durable_message])
    member_two_recruited = (
        "Starter Colony member 2: status=active, role=guard, "
        "order=follow, generation=5."
    )
    issue_command("colony status", [member_one_held, member_two_recruited])

    follow_before_move = call_and_record(
        client,
        transcript,
        "list_entities",
        {"radius": 24.0, "limit": 64},
    ...
    follow_candidates = visible_villagers(follow_before_move)
    if len(follow_candidates) < 2:
        raise RuntimeError("recruited villagers were not visible before follow check")
    call_and_record(
        client,
        transcript,
        "move_by",
    ...
    follow_at_new_position = call_and_record(
        client,
        transcript,
        "list_entities",
        {"radius": 24.0, "limit": 64},
    ...
    follow_at_new_position_by_id = {
        int(entity["entity_id"]): entity
        for entity in visible_villagers(follow_at_new_position)
    }
    issue_command("colony order follow 2", ["Applied Luau order follow."])
    follow_after = call_and_record(
        client,
        transcript,
        "list_entities",
        {"radius": 24.0, "limit": 64},
    ...
    follow_motion_candidates: list[dict[str, Any]] = []
    for entity in visible_villagers(follow_after):
        entity_id = int(entity["entity_id"])
        before = follow_at_new_position_by_id.get(entity_id)
        if before is None:
            continue
        dx = float(entity["x"]) - float(before["x"])
        dz = float(entity["z"]) - float(before["z"])
        follow_motion_candidates.append(
            {
                "entity_id": entity_id,
                "entity_uuid": entity["entity_uuid"],
                "entity_type": entity["entity_type"],
                "horizontal_distance": (dx * dx + dz * dz) ** 0.5,
                "distance_to_player_before": before["distance"],
                "distance_to_player_after": entity["distance"],
                "start_x": before["x"],
                "start_z": before["z"],
                "end_x": entity["x"],
                "end_z": entity["z"],
    ...
    if not follow_motion_candidates:
        raise RuntimeError("no villager identity survived the follow observation")
    follow_motion = max(
        follow_motion_candidates,
        key=lambda motion: float(motion["horizontal_distance"]),
    )
    if (
        float(follow_motion["horizontal_distance"]) < 0.25
        or float(follow_motion["distance_to_player_after"])
        >= float(follow_motion["distance_to_player_before"])
    ):
        raise RuntimeError(
            "no rebound villager approached the moved player after follow refresh: "
            f"{follow_motion}"
        )
    call_and_record(
        client,
    ...
    screenshot = capture_screenshot(
        ...
        run_dir,
        screenshots_dir,
        f"{scenario_id}-follow",
        timeout_seconds,
    )

    exploratory_checks.append(
        {
            "check": "unsupported role",
            "command": "/colony role lumberjack 1",
            "observation": issue_command(
                "colony role lumberjack 1",
                [invalid_action],
            ),
        }
    )
    exploratory_checks.append(
        {
            "check": "unsupported order",
            "command": "/colony order patrol 1",
            "observation": issue_command(
                "colony order patrol 1",
                [invalid_action],
            ),
        }
    )
    exploratory_checks.append(
        {
            "check": "out-of-range dismiss",
            "command": "/colony dismiss 99",
            "observation": issue_command(
                "colony dismiss 99",
                [invalid_action],
            ),
    ...
    exploratory_checks.append(
        {
            "check": "dismiss empty slot",
            "command": "/colony dismiss 3",
            "observation": issue_command(
                "colony dismiss 3",
                ["Update rejected: member 3 is not active."],
            ),
        }
    )
    away_observation = observe_client()
    away_player = away_observation.get("player")
    if not isinstance(away_player, dict):
        raise RuntimeError("client observation did not contain player coordinates")
    away_x = int(round(float(away_player["x"]))) + 64
    away_y = int(round(float(away_player["y"])))
    away_z = int(round(float(away_player["z"]))) + 64
    issue_command(
        f"tp {away_x} {away_y} {away_z}",
        [f"Teleported to {away_x} {away_y} {away_z}"],
    )
    exploratory_checks.append(
        {
            "check": "recruit with no unbound villager nearby",
            "command": "/colony recruit worker",
            "observation": issue_command(
                "colony recruit worker",
                ["Villager binding failed: not_found."],
            ),
        }
    )

    final_state = call_and_record(client, transcript, "state", {}, timeout_seconds)
    final_observation = observe_client()
    call_and_record(client, transcript, "disconnect", {}, timeout_seconds)
    scenario_report = {
        "result": "passed",
        "id": scenario_id,
        "requested_command_contract_passed": requested_command_contract_passed,
        "exact_commands": exact_commands,
        "command_observations": command_observations,
        "initial_observation": initial_observation,
        "summoned_villagers": summoned_villagers,
        "post_dismiss_member_2_status_count": member_two_count_after,
        "rerecruit_generation": 5,
        "follow_before_move": follow_before_move,
        "follow_motion": follow_motion,
        "follow_after": follow_after,
        "exploratory_checks": exploratory_checks,
        "final_observation": final_observation,
    }
    return "passed", final_state, [screenshot], scenario_report


def run_settlement_chain_scenario(
    client: AgentClient,
    run_dir: Path,
    scenario_id: str,
    server_addr: str,
    ...
    """Drive the server-only settlement chain through one real client.

    Every assertion is on observed chat or a client-visible block scan: site
    adoption, the plot survey, the committed building, and the persistent
    resident. Nothing here inspects plugin or core internals.
    """
    screenshots_dir = run_dir / "screenshots"
    screenshots_dir.mkdir(parents=True, exist_ok=True)
    commands: list[str] = []
    ...
    def observe_client() -> dict[str, Any]:
        observation = call_and_record(client, transcript, "observe", {}, timeout_seconds)
        if not observation.get("in_play"):
            raise RuntimeError(f"client left Play state during {scenario_id}")
        return observation

    def await_state(observation: dict[str, Any], budget: float) -> None:
        state_version = observation.get("state_version")
        if not isinstance(state_version, int):
            raise RuntimeError("client observation did not contain integer state_version")
        # The deadline-driven callers can pass a budget below the client's
        # accepted 0.1 s floor; clamp instead of letting the RPC reject the call
        # with "timeout_seconds must be between 0.1 and 120.0" and turn an
        # ordinary timeout into an exception.
        event_timeout = max(0.1, min(budget, 10.0))
        call_and_record(
            client,
            transcript,
            "wait_state_change",
    ...
    def wait_for_chat(
        ...
        while True:
            ...
            for line in lines:
                ...
                if counts[line] > baseline_counts.get(line, 0):
                    fresh.append(line)
            matched = matcher(fresh)
            if matched is not None:
                return observation, matched
            remaining = deadline - time.monotonic()
            if remaining <= 0.0:
                raise TimeoutError(
                    f"{description} was not observed after the command; "
                    f"new chat lines: {fresh}; last chat batch: {lines}"
                )
            await_state(observation, remaining)
    ...
    def issue_until(command: str, matcher: Any, description: str) -> Any:
        # A stage advances only on a receipt observed after this submission. The
        # pre-command chat is snapshotted so a line still visible from an earlier
        # stage can never be mistaken for the new answer, and a command the
        # ingress bucket dropped produces no fresh line and times out here.
        before = recent_chat(observe_client())
        baseline_counts = {line: before.count(line) for line in set(before)}
        issue(command)
        observation, matched = wait_for_chat(matcher, description, baseline_counts)
        chat_observations.append(
            {"command": "/" + command, "observed_chat": recent_chat(observation)}
        )
        return matched

    def line_containing(needle: str) -> Any:
        def matcher(lines: list[str]) -> Any:
            for line in lines:
                if needle in line:
                    return line
            return None
    ...
    def player_block_position(observation: dict[str, Any]) -> tuple[int, int, int]:
        player = observation.get("player")
        if not isinstance(player, dict):
            raise RuntimeError("client observation did not contain player coordinates")
        return (
            int(float(player["x"]) // 1),
            int(float(player["y"]) // 1),
            int(float(player["z"]) // 1),
        )

    def site_matcher(lines: list[str]) -> Any:
        found = [
            match
            for match in (SETTLEMENT_SITE_CHAT_LINE.search(line) for line in lines)
            if match is not None
        ]
        return found or None

    def project_matcher(lines: list[str]) -> Any:
        for line in lines:
            match = SETTLEMENT_PROJECT_CHAT_LINE.search(line)
            if match is not None:
                return match
        return None

    def commit_matcher(lines: list[str]) -> Any:
        for line in lines:
            match = SETTLEMENT_COMMIT_CHAT_LINE.search(line)
            if match is not None:
                return match
        return None

    def info_matcher(lines: list[str]) -> Any:
        for line in lines:
            match = SETTLEMENT_INFO_CHAT_LINE.search(line)
            if match is not None:
                return match
        return None
    ...
    call_and_record(client, transcript, "ping", {}, timeout_seconds)
    play_state = wait_for_existing_or_explicit_connection(
        client, transcript, server_addr, timeout_seconds
    )
    if not is_in_play(play_state):
        raise RuntimeError("client did not reach Play state for the settlement chain")

    issue_until(
        "gamemode creative",
        line_containing("Set game mode to Creative"),
        "creative game mode before the long teleport",
    )
    issue_until(
        f"settlement create {SETTLEMENT_CHAIN_NAME} small",
        line_containing(f"Founded {SETTLEMENT_CHAIN_NAME} (small hamlet)."),
        "settlement founding",
    )
    site_candidates = issue_until(
        f"settlement site {SETTLEMENT_CHAIN_NAME}",
        site_matcher,
        "deterministic site candidates",
    )
    # Prefer a village candidate (more home points of interest for populate);
    # otherwise the first core-ordered candidate keeps the run deterministic.
    chosen_site = next(
        (candidate for candidate in site_candidates if candidate.group(2) == "village"),
        site_candidates[0],
    )
    site_id = chosen_site.group(1)
    site_variant = chosen_site.group(2)
    site_origin = (
        int(chosen_site.group(3)),
        int(chosen_site.group(4)),
        int(chosen_site.group(5)),
    )
    issue_until(
        f"settlement adopt {SETTLEMENT_CHAIN_NAME} {site_id}",
        line_containing(f"Adopted {site_id} ("),
        "site adoption",
    )

    teleport_x = site_origin[0]
    teleport_z = site_origin[2]
    issue_until(
        f"tp {teleport_x} {SETTLEMENT_CHAIN_TELEPORT_Y} {teleport_z}",
        line_containing("Teleported to "),
        "teleport to the adopted site",
    )
    landing_deadline = time.monotonic() + timeout_seconds
    previous_y: int | None = None
    anchor: tuple[int, int, int] | None = None
    while anchor is None:
        observation = observe_client()
        position = player_block_position(observation)
        if previous_y == position[1] and position[0] == teleport_x and position[2] == teleport_z:
            try:
                ground = call_and_record(
                    client,
                    transcript,
                    "read_block",
                ...
                ground = None
            if ground is not None and ground.get("is_air") is False:
                anchor = position
                break
        previous_y = position[1]
        remaining = landing_deadline - time.monotonic()
        if remaining <= 0.0:
            raise TimeoutError(f"client did not land at the adopted site: {observation}")
        await_state(observation, remaining)

    issue_until(
        f"settlement survey {SETTLEMENT_CHAIN_NAME} plot",
        line_containing("Survey settlement: "),
        "plot survey",
    )
    survey_observation = observe_client()
    survey_lines = [
        line for line in recent_chat(survey_observation) if "Survey settlement: " in line
    ]
    if not survey_lines or not any("chunks=loaded" in line for line in survey_lines):
        raise RuntimeError(f"plot survey did not run on loaded chunks: {survey_lines}")

    projected = issue_until(
        f"settlement project {SETTLEMENT_CHAIN_NAME} {SETTLEMENT_CHAIN_BLUEPRINT} here",
        project_matcher,
        "building projection",
    )
    building = projected.group(1)
    stage_count = int(projected.group(2))

    for item, total in SETTLEMENT_CHAIN_MATERIALS:
        outstanding = total
        while outstanding > 0:
            batch = min(outstanding, SETTLEMENT_CHAIN_GIVE_CAP)
            issue_until(
                f"give {item} {batch}",
                line_containing(f"Gave {batch} of {item}"),
                f"give {batch} of {item}",
            )
            outstanding -= batch

    issue_until(
        f"settlement fund {SETTLEMENT_CHAIN_NAME} {building}",
        line_containing(f"Reserved real materials for {building} ("),
        "material reservation",
    )
    committed = issue_until(
        f"settlement build {SETTLEMENT_CHAIN_NAME} {building}",
        commit_matcher,
        "building commit",
    )
    committed_building = committed.group(1)
    committed_revision = int(committed.group(2))
    if committed_building != building:
        raise RuntimeError(
            f"commit reported building {committed_building!r}, expected {building!r}"
        )
    info_after_commit = issue_until(
        f"settlement info {SETTLEMENT_CHAIN_NAME}",
        info_matcher,
        "settlement info after the commit",
    )
    houses = int(info_after_commit.group(2))
    if houses < 1:
        raise RuntimeError(
            "settlement info did not report a committed house after the build: "
            f"{info_after_commit.group(0)!r}"
        )

    scan_counts: dict[str, int] = {}
    scan_boxes: list[dict[str, int]] = []
    for min_y, max_y in (
        (anchor[1] - 3, anchor[1] + 2),
        (anchor[1] + 3, anchor[1] + 7),
    ):
        scan_payload = {
            "min_x": anchor[0] - SETTLEMENT_CHAIN_SCAN_HALF_X,
            "min_y": min_y,
            "min_z": anchor[2] - SETTLEMENT_CHAIN_SCAN_HALF_Z,
            "max_x": anchor[0] + SETTLEMENT_CHAIN_SCAN_HALF_X,
            "max_y": max_y,
            "max_z": anchor[2] + SETTLEMENT_CHAIN_SCAN_HALF_Z,
            "max_blocks": 4096,
        }
        scan_result = call_and_record(
            client, transcript, "scan_blocks", scan_payload, timeout_seconds
        )
        blocks = scan_result.get("blocks")
        if not isinstance(blocks, list):
            raise RuntimeError("scan_blocks did not return a block list")
        for block in blocks:
            if not isinstance(block, dict) or not isinstance(block.get("block_id"), str):
                raise RuntimeError("scan_blocks returned a malformed block entry")
            scan_counts[block["block_id"]] = scan_counts.get(block["block_id"], 0) + 1
        scan_boxes.append(scan_payload)
    missing_blocks = [
        block_id
        for block_id in SETTLEMENT_CHAIN_BLOCKS
        if scan_counts.get(block_id, 0) == 0
    ]
    if missing_blocks:
        raise RuntimeError(
            f"committed {SETTLEMENT_CHAIN_BLUEPRINT} was not visible in the anchor scan: "
            f"missing={missing_blocks} counts={sorted(scan_counts.items())}"
        )

    issue_until(
        f"settlement populate {SETTLEMENT_CHAIN_NAME}",
        line_containing(f"settled in {SETTLEMENT_CHAIN_NAME} ("),
        "persistent resident settlement",
    )
    info_after_populate = issue_until(
        f"settlement info {SETTLEMENT_CHAIN_NAME}",
        info_matcher,
        "settlement info after populate",
    )
    population = int(info_after_populate.group(1))
    if population < 1:
        raise RuntimeError(
            "settlement info did not report the settled resident: "
            f"{info_after_populate.group(0)!r}"
        )
    issue_until(
        f"settlement residents {SETTLEMENT_CHAIN_NAME}",
        line_containing("family="),
        "resident listing",
    )
    ...
    scenario_report = {
        "result": "passed",
        "id": scenario_id,
        "settlement": SETTLEMENT_CHAIN_NAME,
        "commands": commands,
        "chat_observations": chat_observations,
        "site_candidates": [candidate.group(0) for candidate in site_candidates],
        "chosen_site": {
            "site_id": site_id,
            "variant": site_variant,
            "origin": list(site_origin),
        },
        "anchor": list(anchor),
        "survey": survey_lines,
        "building": building,
        "building_stages": stage_count,
        "committed_revision": committed_revision,
        "houses": houses,
        "population": population,
        "scan_boxes": scan_boxes,
        "scan_block_counts": dict(sorted(scan_counts.items())),
    }
    return "passed", final_state, [screenshot], scenario_report
