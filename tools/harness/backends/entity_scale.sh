...

{
    ...
    echo "entity_mix=${SOLARIS_ENTITY_BENCH_ENTITY_MIX:-single:${SOLARIS_ENTITY_BENCH_ENTITY_TYPE:-minecraft:husk}}"
    echo "view_distance=${SOLARIS_ENTITY_BENCH_VIEW_DISTANCE:-2}"
    echo "region_spacing_blocks=${SOLARIS_ENTITY_BENCH_REGION_SPACING_BLOCKS:-128}"
    echo "p95_budget_us=${SOLARIS_ENTITY_BENCH_P95_BUDGET_US:-0}"
    echo "p99_budget_us=${SOLARIS_ENTITY_BENCH_P99_BUDGET_US:-0}"
    echo "connect_wave=${SOLARIS_ENTITY_BENCH_CONNECT_WAVE:-4}"
    echo "log_filter=${SOLARIS_ENTITY_BENCH_LOG_FILTER:-mc_net::lock_metrics=warn,mc_net::server=warn}"
