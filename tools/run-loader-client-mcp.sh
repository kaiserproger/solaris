...
usage() {
    printf '%s\n' 'Usage: tools/run-loader-client-mcp.sh <fabric|neoforge|forge> [--check]'
}
...
case "$PLATFORM" in
    ...
    -h|--help)
        ...
        ;;
    *)
        printf 'Unknown Loader platform: %s\n' "$PLATFORM" >&2
        usage >&2
        exit 2
        ;;
esac
...
SOLARIS_CLIENT_MCP_GAME_DIR="${SOLARIS_CLIENT_MCP_GAME_DIR:-$REPO_ROOT/.analysis/minecraft-loader-mcp/$PLATFORM}"
SOLARIS_CLIENT_MCP_USERNAME="${SOLARIS_CLIENT_MCP_USERNAME:-SolarisLoader}"
...
if [[ "$MODE" == "check" ]]; then
    ...
    "$AGENT_ROOT/gradlew" \
        ...
        "-Psolaris.clientMcp.gameDir=$SOLARIS_CLIENT_MCP_GAME_DIR" \
        "-Psolaris.clientMcp.username=$SOLARIS_CLIENT_MCP_USERNAME" \
        ":loader-$PLATFORM:runClientMcp"
    printf 'Loader MCP check passed; client was not launched.\n'
    exit 0
fi
...
mkdir -p "$SOLARIS_CLIENT_MCP_GAME_DIR"
