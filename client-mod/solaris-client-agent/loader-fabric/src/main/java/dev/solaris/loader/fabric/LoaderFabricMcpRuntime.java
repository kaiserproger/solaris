...
final class LoaderFabricMcpRuntime {
    ...
    static LoaderFabricMcpRuntime createIfEnabled() {
        return create(
                System.getenv(TOKEN_ENV),
                LoaderFabricMcpRuntime.class.getClassLoader());
    }

    static LoaderFabricMcpRuntime create(String token, ClassLoader loader) {
        ...
        try {
            ...
            Class<?> stateEvents = Class.forName(
                    ...
                    true,
                    loader);
            return new LoaderFabricMcpRuntime(
            ...
            throw new IllegalStateException(
                    "Fabric Loader MCP runtime classes are unavailable",
                    error);
        }
    }
    ...
    void afterTick() {
        ...
        if (!started) {
            ...
            if (!Boolean.TRUE.equals(result)) {
                throw new IllegalStateException("Fabric Loader MCP runtime did not start");
            }
            started = true;
        }
    }
    ...
    private static Object invoke(Method method) {
        try {
            ...
            throw new IllegalStateException(
                    "Fabric Loader MCP runtime method is inaccessible",
                    error);
            ...
            throw new IllegalStateException(
                    "Fabric Loader MCP runtime method failed",
                    cause);
        }
    }
}
